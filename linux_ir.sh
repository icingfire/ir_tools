...
#  安全声明：
#    - 脚本默认只读采集，不对系统做持久性修改；
#    - 部分命令（如 timeline, journalctl）可能较重，请在业务低峰执行；
...
copy_files_with_structure() {
    local src_file_list="$1"
    local dst_path="$2"

    # 参数检查
    if [[ ! -f "$src_file_list" ]]; then
        echo "源文件列表不存在: $src_file_list" >&2
        return 1
    fi
    if [[ -z "$dst_path" ]]; then
        echo "目标路径未指定" >&2
    ...
    while IFS= read -r file; do
        ...
        if [[ -f "$file" ]]; then
            mkdir -p "$dst_path/$(dirname "$file")" || continue
            cp -a "$file" "$dst_path/$file"
        else
            echo "文件不存在: $file" >&2
        fi
    done < "$src_file_list"
...
init_out_dir() {
  ...
  mkdir -p "$OUT_DIR/meta" "$OUT_DIR/sysinfo" "$OUT_DIR/process" "$OUT_DIR/network" \
           ...
           "$OUT_DIR/cloud" "$OUT_DIR/forensic" "$OUT_DIR/security" "$OUT_DIR/tmp" "$OUT_DIR/timeline" || true
...
collect_accounts() {
  ...
  IFS=$' \t\n'
  awk -F: '{print $1" "$3" "$6}' /etc/passwd | while read -r USERNAME USERID TMP_HOME; do
    [[ -z "$TMP_HOME" || ! -d "$TMP_HOME" ]] && continue
    SAFE_USER=$(echo "$USERNAME" | tr -cd 'A-Za-z0-9._-')
    UDIR="$OUT_DIR/accounts/users/$SAFE_USER"
    mkdir -p "$UDIR"
    save_cmd_output "$UDIR/id.txt" bash -lc "id $USERNAME"
    save_file_safe "$TMP_HOME/.bash_history" "$UDIR/bash_history" || true
    save_file_safe "$TMP_HOME/.zsh_history" "$UDIR/zsh_history" || true
    save_file_safe "$TMP_HOME/.ash_history" "$UDIR/ash_history" || true
    save_file_safe "$TMP_HOME/.python_history" "$UDIR/python_history" || true
    save_file_safe "$TMP_HOME/.mysql_history" "$UDIR/mysql_history" || true
    save_file_safe "$TMP_HOME/.psql_history" "$UDIR/psql_history" || true

    save_tree_safe "$TMP_HOME/.ssh" "$UDIR/ssh"
    save_tree_safe "$TMP_HOME/.config" "$UDIR/config"
    save_tree_safe "$TMP_HOME/.profile.d" "$UDIR/profile.d"
    save_file_safe "$TMP_HOME/.profile" "$UDIR/.profile"
    save_file_safe "$TMP_HOME/.bashrc" "$UDIR/.bashrc"
    save_file_safe "$TMP_HOME/.zshrc" "$UDIR/.zshrc"

    # 可疑 authorized_keys 中的注释/命令限制
    if [[ -f "$TMP_HOME/.ssh/authorized_keys" ]]; then
      grep -nE 'command=|from=|no-pty|no-port-forwarding|permitopen' "$TMP_HOME/.ssh/authorized_keys" >"$UDIR/ssh/authorized_keys_flags.txt" 2>/dev/null || true
    fi
  done
  IFS=$'\n\t'
...
#--------------------------------------
create_timeline() {
  if [ ${TIMELINE_FLAG} -eq 0 ]; then
    return
  fi
  
  time_bin=$(ls -1 timeline*|head -1)
  if [ -n "${time_bin}"  -a -f "${time_bin}" ]; then
    if [ ! -x "${time_bin}"  ]; then
	  chmod +x "${time_bin}"
    fi
  else
    warn "timeline binary not found"
	TIMELINE_FLAG=0
    return
  fi
  
  info "timeline创建已启动"
  ./"${time_bin}" -o "$OUT_DIR/timeline/" -S &
}
...
wait_for_timeline() {
  ...
  timeline_process=$(ps -eo cmd |grep timeline|grep -v grep)
  if [ -n "${timeline_process}" ]; then
    info "等待timeline结束..."
    while true; do
      timeline_process=$(ps -eo cmd |grep timeline|grep -v grep)
      [ -z "${timeline_process}" ] && break
      for dots in "." ".." "..." "...." "....."; do
        echo -ne "\r$dots"
        sleep 0.5
      done
...
collect_logs() {
  ...
  # /var/log 目录树
  TMP_LOG_FILE=t_log_list.txt
  find /var/log -ctime -${RECENT_DAYS} -type f | grep -v journal > ${TMP_LOG_FILE}
  ...
  if [[ $INCLUDE_APP_LOGS -eq 1 ]]; then
    for p in /opt /usr/local /data /srv; do
      ...
      find "$p" -maxdepth 3 -type d -name log -o -name logs 2>/dev/null | head -n 200 | while read -r d; do
	    find "$d" -ctime -${RECENT_DAYS} -type f >> ${TMP_LOG_FILE}
      done
    done
  fi
  
  copy_files_with_structure ${TMP_LOG_FILE} "$OUT_DIR/logs"
  rm -f ${TMP_LOG_FILE}
...
collect_kernel_modules() {
  ...
  awk 'NR>1{print $1}' "$OUT_DIR/sysinfo/lsmod.txt" 2>/dev/null | sort -u | while read -r m; do
    ...
    if [[ -f "$MODPATH" ]]; then
      #save_file_safe "$MODPATH" "$MD/$(basename "$MODPATH")"    # 减少取证size，计算hash即可
      if [[ $ENABLE_HASH -eq 1 ]] && need_cmd "$HASH_CMD"; then
        save_cmd_output "$MD/hash.txt" "$HASH_CMD" "$MODPATH"
      fi
	else
	  info "Can not find ko: ${MODPATH}"
    fi
  done
...
collect_pcap() {
  ...
  "$TCPDUMP" -i "$IFACE" -w "$PCAP_FILE" -G "$PCAP_DURATION" -W 1 -nn 2>"$OUT_DIR/network/pcap/tcpdump_${IFACE}.log" &
  ok "抓包完成：$PCAP_FILE"
}
...
main() {
  parse_args "$@"
  init_out_dir "$@"
  read -p "Please input days of logs are collected: " days
  if [[ "$days" =~ ^[0-9]+$ ]]; then
    RECENT_DAYS=${days}
  else
    echo "Invalid input, use default ${RECENT_DAYS} days"
  fi
  probe_os
  create_timeline
  collect_basic_sysinfo
